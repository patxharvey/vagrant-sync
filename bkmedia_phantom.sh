...
function perform_restore {
    ...
    # Informing the user about the restore process
    ...
    # Confirm completion of the restore process
...
function perform_integrity_restore {
    ...
    # Informing the user about the integrity restore process
    echo "Restoring integrity for $user_host:$path"

    # Read the log file line by line
    while IFS= read -r log_line; do
        # Check if the log line indicates a file altered by the phantom
        if [[ "$log_line" == *"File altered by phantom:"* ]]; then
            ...
            # Check if the original file exists in the backup directory
            if [[ -f "$BACKUP_DIR/$original_file" ]]; then
                # Copy the original file back to its location
                cp "$BACKUP_DIR/$original_file" "$user_host:$path/$original_file"
                
                # Inform the user about the restoration
...
case "$1" in
    ...
    -I)
        ...
        ;;
    *)
        # Display usage information if no valid arguments are provided
